...
$query_sql="select * from student WHERE advice!='' order BY id ASC limit $offet,$pagesize";//查询本页的数据
...
//计算总的页数
$pagenum=ceil($count_array['count']/$pagesize)-1;
//循环输出页数及其链接
    ...
    <div class="panel panel-default">
        ...
        <table class="table table-bordered table-hover">
            ...
            <tr>
                ...
                <td><?php echo $list['advice'];?></td>
                <td><?php echo $list['sname'];?></td>
                <td><button id="<?php echo $list['id'] ?>" class="btn btn-danger" onclick="deladv(this)">删除</button></td>
            </tr>
            ...
            <?php
                if ($pagenum>1) {
                    if($p==1){
                        ...
                        echo'<li><a href="lookadv.php?p=',($p-1),'" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>';
                    }
                    for ($i = 1; $i <$pagenum; $i++) {
                        if ($i == $p) {
                            echo '<li class="active"><a href="#">', $i, '<span class="sr-only">(current)</span></a></li>';  //因为是在本业所以不用输出超链接
                        } else {
                            echo '<li><a href="lookadv.php?p=', $i, '">', $i, '</a></li>';
                        }
                    }
                    if($p==$pagenum){
                        echo'<li class="disabled"><a href="#" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>';
                    }else{
                        echo'<li><a href="lookadv.php?p=',($p+1),'" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>';
                    }
                }
            ?>
            </ul>
...
<script>
    function deladv(obj) {
        ...
        var id=obj.id;
        var url="deladv.php?id="+id;
        xmlHttp.open("get",url,true);
        xmlHttp.onreadystatechange=function () {
            if (xmlHttp.readyState == 4 && xmlHttp.status ==200){
                if (xmlHttp.responseText == '1'){
                    alert('删除成功');window.location.href='lookadv.php';
                }
            }
        }
        xmlHttp.send(null);
