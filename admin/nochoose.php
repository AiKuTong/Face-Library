...
$query_sql="select * from choose_seat WHERE state='0' order BY id ASC limit $offet,$pagesize";//查询本页的数据
$res=mysqli_query($con,$query_sql);
//计算总留言数
$count_res=mysqli_query($con,"select count(*) as count from choose_seat ");
        ...
        <div class="panel panel-default">
            ...
            <table class="table table-bordered table-hover">
                <tr>
                    ...
                    <td class="th">学号</td>
                    <td class="th">占座日期</td>
                    <td class="th">占座具体时间</td>
                    <td class="th">离座日期</td>
                    <td class="th">离座具体时间</td>
                </tr>
                <tr>
                <?php
                while ($list=mysqli_fetch_array($res))
                { ?>
                <td><?php echo $list['seat_id'];?></td>
                <td><?php echo $list['snum'];?></td>
                <td><?php echo $list['choose_day'];?></td>
                <td><?php echo $list['choose_hour'];?></td>
                <td><?php echo $list['leave_day'];?></td>
                <td><?php echo $list['leave_hour'];?></td>
            </tr>
            <?php } ?>
            </table>
        </div>
            ...
            <?php
                if ($pagenum>1) {
                    if($p==1){
                        ...
                        echo'<li><a href="nochoose.php?p=',($p-1),'" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>';
                    }
                    for ($i = 1; $i <=$pagenum; $i++) {
                        if ($i == $p) {
                            echo '<li class="active"><a href="#">', $i, '<span class="sr-only">(current)</span></a></li>';  //因为是在本业所以不用输出超链接
                        } else {
                            echo '<li><a href="nochoose.php?p=', $i, '">', $i, '</a></li>';
                        }
                    }
                    if($p==$pagenum){
                        echo'<li class="disabled"><a href="#" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>';
                    }else{
                        echo'<li><a href="nochoose.php?p=',($p+1),'" aria-label="Next"><span aria-hidden="true">&raquo;</span></a></li>';
                    }
                }
            ?>
            </ul>
